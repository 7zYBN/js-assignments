...
function getRectangleString(width, height) {
    //console.log(`┌${'─'.repeat(width - 2)}┐\n` + `│${' '.repeat(width - 2)}│`.repeat(height - 2).repeat(height) + '\n' + `└${'─'.repeat(width - 2)}┘\n`)
...
function encodeToRot13(str) {
    ...
    return str.split('').map(item => {
        if (input.indexOf(item.toLowerCase()) === -1) {
            return item;
        }
        return item === item.toUpperCase() ? input[(input.indexOf(item.toLowerCase()) + 13) % input.length].toUpperCase() : input[(input.indexOf(item) + 13) % input.length];
    }).join('');
